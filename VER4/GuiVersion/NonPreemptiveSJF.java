...
public class NonPreemptiveSJF extends Scheduler {
    ...
    public void process() {
        ...
        while (!processList.isEmpty()|| !readyQueue.isEmpty()) {
            if (!readyQueue.isEmpty()) {
                ...
                Process currentProcess = selectShortestJob();
                int startTime = Math.max(currentTime, currentProcess.getStartTime());
                int finishTime = startTime + currentProcess.getFinishTime();
                this.getOutputList().add(new Process(currentProcess.getProcessName(), startTime, finishTime));
				
                //check if all processes is completed
				for (Process inputProcess : this.getInputList()) {
					if (inputProcess.getProcessName().equals(currentProcess.getProcessName())) {
						inputProcess.setEndStatus(true);
				...
				allProcessesCompleted = true; // Initialize to true
				for (Process p : this.getInputList()) {
					if (!p.getEndStatus()) { // If endStatus is false for any process
						allProcessesCompleted = false;
						break; // Exit the loop as we found a process that hasn't completed
					}
				}

			//	for (Process p : this.getInputList()) {
			//		System.out.println(p.getProcessName());
			//		System.out.println(p.getEndStatus());
			//	}

				//System.out.println("dsdd"+allProcessesCompleted);
			
                currentTime = finishTime; // Update current time to the finish time of the current process
				if(readyQueue.isEmpty() && !allProcessesCompleted){
					this.getOutputList().add(new Process("--", currentTime, currentTime+1));
					continue; 
				}
                ...
                if (!processList.isEmpty()) {
                    currentTime = processList.get(0).getStartTime();
    ...
    private Process selectShortestJob() {
		Process shortestJob = Collections.min(readyQueue, Comparator.comparingInt(Process::getFinishTime));
        return shortestJob;
    }
